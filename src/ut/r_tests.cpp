...
TEST(r_tests, exp) {
  ...
  {
    // exp(-2) ~ 0.13533528323661269189399949497248...
  ...
  // Generated by AI — regression: large positive arguments must keep every
  // integer digit. exp_rational's working precision must exceed the base-4
  // magnitude of e^|m| (~0.72135*|m| digits); the previous |m|/2 estimate was
  // too small, so e.g. exp(200) was wrong from about the 52nd digit.
  {
    // exp(150) ~ 1.3937e65
    auto x = epx::make_q(stosz("150"), stosz("1"));
    EXPECT_EQ("139370958066637969731834193714145747747369006140218438233756444835.68081931",
              epx::to_string(epx::exp(x), 8));
  }
  {
    // exp(200) ~ 7.226e86 (87-digit integer part)
    auto x = epx::make_q(stosz("200"), stosz("1"));
    EXPECT_EQ(
        "722597376812574925817747704218930569735687442852731928403269789123221909361473891661561.92659",
        epx::to_string(epx::exp(x), 5));
  }
  {
    // exp(300) ~ 1.9424e130 (130-digit integer part)
    auto x = epx::make_q(stosz("300"), stosz("1"));
    EXPECT_EQ(
        "194242639524125593658420883601769921936620862195160469414291771806713452728791826196664368"
        "40448422418235826784451770832010132261535.3130",
        epx::to_string(epx::exp(x), 4));
  }
  {
    // exp(-20) ~ 2.061e-9
    auto x = epx::make_q(stosz("-20"), stosz("1"));
...
TEST(r_tests, pow) {
  ...
  // Generated by AI — regression: large integer exponents must be exact.
  // pow(b, e) = exp(e * log(b)); for large e the internal exp_rational must
  // carry enough working precision to keep every digit of e^|m| (m ~ e*log b).
  // Previously pow(2, 1000) matched only ~232 of its 302 digits.
  {
    auto two = epx::make_q(stosz("2"), stosz("1"));
    auto e100 = epx::make_q(stosz("100"), stosz("1"));
    EXPECT_EQ("1267650600228229401496703205376", epx::to_string(epx::pow(two, e100), 0));
  }
  {
    auto two = epx::make_q(stosz("2"), stosz("1"));
    auto e256 = epx::make_q(stosz("256"), stosz("1"));
    EXPECT_EQ("115792089237316195423570985008687907853269984665640564039457584007913129639936",
              epx::to_string(epx::pow(two, e256), 0));
  }
  {
    auto two = epx::make_q(stosz("2"), stosz("1"));
    auto e500 = epx::make_q(stosz("500"), stosz("1"));
    EXPECT_EQ(
        ...
        epx::to_string(epx::pow(two, e500), 0));
  }
  {
    // Headline regression case: pow(2, 1000) = 2^1000 exactly (302 digits).
    auto two = epx::make_q(stosz("2"), stosz("1"));
    auto e1000 = epx::make_q(stosz("1000"), stosz("1"));
    EXPECT_EQ(
        "107150860718626732094842504906000181056140481170553360744375038837035105112493612249319837"
        "88156958581275946729175531468251871452856923140435984577574698574803934567774824230985421"
        "07460506237114187795418215304647498358194126739876755916554394607706291457119647768654216"
        "7660429831652624386837205668069376",
        epx::to_string(epx::pow(two, e1000), 0));
  }
  {
    // pow(2, 1000) is integer-valued: the fractional part must be all zeros.
    auto two = epx::make_q(stosz("2"), stosz("1"));
    auto e1000 = epx::make_q(stosz("1000"), stosz("1"));
    auto s = epx::to_string(epx::pow(two, e1000), 6);
    EXPECT_EQ(".000000", s.substr(s.size() - 7));
  }
  {
    // pow(3, 200) exact.
    auto three = epx::make_q(stosz("3"), stosz("1"));
    auto e200 = epx::make_q(stosz("200"), stosz("1"));
    EXPECT_EQ("265613988875874769338781322035779626829233452653394495974574961739092490901302182994384"
              "699044001",
              epx::to_string(epx::pow(three, e200), 0));
  }
  {
    // Cross digit size (mz): pow(2, 300) exact.
    auto two = epx::make_q(stomz("2"), stomz("1"));
    auto e300 = epx::make_q(stomz("300"), stomz("1"));
    EXPECT_EQ("203703597633448608626844568840937816105146839366593625063614044935438129976333670618339"
              "7376",
              epx::to_string(epx::pow(two, e300), 0));
  }
  {
    // Identity: pow(2, 1000) * pow(2, -1000) = 1 (exercises the negative branch).
    auto two = epx::make_q(stosz("2"), stosz("1"));
    auto e1000 = epx::make_q(stosz("1000"), stosz("1"));
    auto neg1000 = epx::make_q(stosz("-1000"), stosz("1"));
    auto id = epx::mul(epx::pow(two, e1000), epx::pow(two, neg1000));
    EXPECT_EQ("1.00000000000000000000", epx::to_string(id, 20));
  }
}
